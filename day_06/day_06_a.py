...
with open(r'real_data.txt') as f:
    ...
    times = [int(x.strip()) for x in time_line.split(':')[1].split(' ') if x]
    distances = [int(x.strip()) for x in distance_line.split(':')[1].split(' ') if x]
...
print(f'Answer: {math.prod(results)}') # 275724